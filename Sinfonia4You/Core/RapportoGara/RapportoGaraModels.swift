...
struct SessioneRapportoGara: Codable, Identifiable, Hashable {
    ...
    var distinte: DistinteSessioneRapportoGara
    var avviataIl: Date
    ...
    init(
        ...
        distinte: DistinteSessioneRapportoGara = .empty,
        avviataIl: Date,
    ...
    init(from decoder: Decoder) throws {
        ...
        distinte = try container.decodeIfPresent(DistinteSessioneRapportoGara.self, forKey: .distinte) ?? .empty
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(distinte, forKey: .distinte)
    ...
    func renderizzataLive(alla data: Date) -> SessioneRapportoGara {
        guard statoCronometro.isInCorso else { return self }

        let delta = max(0, Int(data.timeIntervalSince(aggiornataIl)))
        guard delta > 0 else { return self }

        var renderizzata = self
        ...
        renderizzata.aggiornataIl = data
        return renderizzata
    }

    func cronometroDisplayLive(alla data: Date) -> CronometroDisplayLiveRapportoGara {
        let secondiClamped = max(0, secondiCronometroPrecisi(alla: data))
        let secondiInteri = Int(floor(secondiClamped))
        let minuti = secondiInteri / 60
        let residuo = secondiInteri % 60
        let decimi = statoCronometro.isInCorso
            ? min(9, Int((secondiClamped - floor(secondiClamped)) * 10))
            : nil
    ...
    private func secondiCronometroPrecisi(alla data: Date) -> TimeInterval {
        ...
        if statoCronometro.isInCorso {
            deltaAttivo = max(0.1, data.timeIntervalSince(aggiornataIl))
        } else {
            deltaAttivo = 0
        }

        switch statoCronometro {
        case .prepartita:
            return 0
        case .primoTempo, .recuperoPrimoTempo, .intervallo:
            return TimeInterval(secondiPrimoTempo) + deltaAttivo
        case .secondoTempo, .recuperoSecondoTempo, .finale:
            return TimeInterval(secondiSecondoTempo) + deltaAttivo
        }
    }
...
struct CronometroDisplayLiveRapportoGara: Hashable {
    var principale: String
    var decimi: Int?
}

struct AppleWatchAbbinatoRapportoGara: Codable, Identifiable, Hashable {
    let id: UUID
    var watchIdentifier: String
    var watchName: String
    var abbinatoIl: Date
    var ultimoSyncIl: Date?

    init(
        id: UUID = UUID(),
        watchIdentifier: String,
        watchName: String,
        abbinatoIl: Date = Date(),
        ultimoSyncIl: Date? = nil
    ) {
        self.id = id
        self.watchIdentifier = watchIdentifier
        self.watchName = watchName
        self.abbinatoIl = abbinatoIl
        self.ultimoSyncIl = ultimoSyncIl
    }
}

struct CodiceVerificaRapportoGara: Codable, Hashable {
    var codice: String
    var generatoIl: Date
    var scadeIl: Date
    ...
    func isValido(at referenceDate: Date) -> Bool {
        referenceDate < scadeIl
    }

    func secondiResidui(at referenceDate: Date) -> Int {
        max(0, Int(scadeIl.timeIntervalSince(referenceDate).rounded(.down)))
    }

    func labelTempoResiduo(at referenceDate: Date) -> String {
        let secondi = secondiResidui(at: referenceDate)
...
enum StatoRegistrazioneAudioRapportoGara: String, Codable, CaseIterable {
    case inAttesa
    case trascritta
    case errore
...
struct RegistrazioneAudioRapportoGara: Codable, Identifiable, Hashable {
    let id: UUID
    var sessionID: UUID
    var requestID: UUID
    var snapshot: MinutoRapportoGaraSnapshot?
    ...
    var creatoIl: Date
    var byteCount: Int
    var testoTrascritto: String?
    var ultimoErrore: String?
    var stato: StatoRegistrazioneAudioRapportoGara

    init(
        id: UUID = UUID(),
        sessionID: UUID,
        requestID: UUID,
        snapshot: MinutoRapportoGaraSnapshot? = nil,
        fileName: String,
        creatoIl: Date = Date(),
        byteCount: Int,
        testoTrascritto: String? = nil,
        ultimoErrore: String? = nil,
        stato: StatoRegistrazioneAudioRapportoGara = .inAttesa
    ) {
        self.id = id
        self.sessionID = sessionID
        ...
        self.testoTrascritto = testoTrascritto
        self.ultimoErrore = ultimoErrore
        self.stato = stato
    }
}
...
extension SessioneRapportoGara {
    func merging(with incoming: SessioneRapportoGara) -> SessioneRapportoGara {
        var mergedEventsByID = Dictionary(uniqueKeysWithValues: eventi.map { ($0.id, $0) })
        for event in incoming.eventi {
            mergedEventsByID[event.id] = event
        }

        return SessioneRapportoGara(
            id: incoming.id,
            sessionId: incoming.sessionId,
            designazioneId: incoming.designazioneId.isEmpty ? designazioneId : incoming.designazioneId,
            titoloGara: incoming.titoloGara.isEmpty ? titoloGara : incoming.titoloGara,
            dataGara: incoming.dataGara.isEmpty ? dataGara : incoming.dataGara,
            ruoloLabel: incoming.ruoloLabel.isEmpty ? ruoloLabel : incoming.ruoloLabel,
            nomeOrologio: incoming.nomeOrologio.isEmpty ? nomeOrologio : incoming.nomeOrologio,
            coloreMagliaCasa: incoming.coloreMagliaCasa.isEmpty ? coloreMagliaCasa : incoming.coloreMagliaCasa,
            coloreMagliaOspiti: incoming.coloreMagliaOspiti.isEmpty ? coloreMagliaOspiti : incoming.coloreMagliaOspiti,
            statoCronometro: incoming.statoCronometro,
            secondiPrimoTempo: max(secondiPrimoTempo, incoming.secondiPrimoTempo),
            secondiSecondoTempo: max(secondiSecondoTempo, incoming.secondiSecondoTempo),
            minutiRecuperoPrimoTempo: incoming.minutiRecuperoPrimoTempo ?? minutiRecuperoPrimoTempo,
            minutiRecuperoSecondoTempo: incoming.minutiRecuperoSecondoTempo ?? minutiRecuperoSecondoTempo,
            eventi: Array(mergedEventsByID.values),
            distinte: incoming.distinte.hasContent ? incoming.distinte : distinte,
            avviataIl: min(avviataIl, incoming.avviataIl),
            aggiornataIl: max(aggiornataIl, incoming.aggiornataIl),
            sincronizzataIl: incoming.sincronizzataIl ?? sincronizzataIl
        )
    }
}
